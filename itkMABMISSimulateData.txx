...
int
MABMISSimulateData<TInputImage, TOutputImage>
:: DoPCATraining(std::vector<std::string> deformationFieldFileNames, int numFiles, std::vector<std::string> allImgFileName, int root)
{
	...
	if (1) // if (0) // if (1)
	{
		...
		InternalImageType::Pointer templateImage = 0;
	 	...
	 	for (int i = 0; i < numAllCombinations; i++)	{
			...
			intermediateSubDeformationFieldFileName.erase (intermediateSubDeformationFieldFileName.end()-7, intermediateSubDeformationFieldFileName.end());
			...
			intermediateDeformationFieldFileName.erase (intermediateDeformationFieldFileName.end()-7,intermediateDeformationFieldFileName.end());
			...
			intermediateReversedDeformationFieldFileName.erase (intermediateReversedDeformationFieldFileName.end()-7, intermediateReversedDeformationFieldFileName.end());
			intermediateReversedDeformationFieldFileName.append(index_string);
			intermediateReversedDeformationFieldFileName.append(".mha");
			intermediateReversedDeformationFieldFileName = tempFolder + FILESEP + intermediateReversedDeformationFieldFileName;
		...
		// do selection
		{
			...
			delete[] index;
			for (int i = 0; i < m_AtlasSize; i++) delete[] dist[i];
			delete[] dist;
