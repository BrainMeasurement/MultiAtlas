			 ...
			 std::vector<int> iterations, double sigma)
{
	...
	//if (!isDebug)	if (false)
	{
		for (int i = 0; i < imageFiles.size(); i++)
		{
			...
			std::string subdeformationFieldFileName = std::string(i_str) + "_to_" + std::string(root_str) + "deform_000_sub.mha";
	...
	tree = treeoperator->BuildCombinativeTree(root, atlasfilenames, tree_size, tree); //cout << "pass: BuildCombinativeTree " << endl;
	...
	for (int i=0; i<tree_size; i++) atlas.m_Tree[i] = tree[i];
...
template <class T> int DoIt( itk::MABMISImageData* trainingData, std::string outputFile, 
							  std::vector<int> iterations, double sigma)
{  
