...
// basic image type
  ...
  vnl_vector<int> BuildCombinativeTree(int root, std::vector<std::string> atlasfilenames, int tree_size, vnl_vector<int> tree);
  vnl_vector<int>  BuildIncrementalTree(int sample_size, int tree_size, int allfilenumber,  int* sample_cur, int* atlas_cur, vnl_matrix<double> cross_dist, vnl_vector<int> itree);
  vnl_vector<int> generateMSTFromMatrix(vnl_matrix<double> curDistance, int nnode, vnl_vector<int> treeMST );
  vnl_vector<int> generateMSTFromMatrixWithFixedRoot(vnl_matrix<double> curDistance, int nnode, vnl_vector<int> treeMST, int root );
  ...
  vnl_vector<int> FromEdgeSegment2Tree(vnl_vector<int> tree, bool* treeEdgeUsed, int* v1, int* v2, int curRoot, int matrixSize);
